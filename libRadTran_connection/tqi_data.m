...
dates = {};
hours = linspace(6, 18, 13);
hours = linspace(3, 22, 20);
minutes = linspace(0, 45, 4);
minutes = linspace(0, 55, 12)
date_stub = '2023 08 01';
for h = hours
    H = num2str(h);
    if numel(H) == 1
        H = ['0', H];
...
for d = 1:numel(dates)
    ...
    for v = 1:numel(visibilities)
        ...
        %outq = p.output_quantity.OptionResultToVariable('transmittance');
        inp = [ ...
            '~/Documents/conference_data/inputs/time_', ...
            replace(D, ' ', '_'), '__', ...
