...
% 1.1.1 source
% for details see
% "Design and test of optical payload for polarization encoded QKD for Nanosatellites" by Sagar, Jaya, Hastings, Elliott, Zhang, Piede, Stefko, Milan, Lowndes, David, Oi, Daniel, Rarity, John, Joshi, Siddarth K.
...
% 1.1.2 Telescope
% for details (and details of downlink beacon) see Quarc: Quantum research cubesat—a constellation for quantum communication Mazzarella, Luc, Lowe, Christopher, Lowndes, David, Joshi, Siddarth Koduru Greenland, Steve McNeil, Doug, Mercury, Cassandra, Macdonald, Malcolm, Rarity, John, Oi, Daniel Kuan Li
...
downlink_beacon_telescope = components.Telescope( ...
    0.01,                                             ... % telescope diameter (m)
    'FOV',                deg2rad(0.4),               ... % telescope field of view (rads)
    'Wavelength',         downlink_beacon_wavelength, ...
    'Optical_Efficiency', 1,                          ... % optical efficiency (unitless)
    'Pointing_Jitter',    1e-3);                      ... % pointing precision (coarse pointing precision)
                                                          % in rads note that to make this calculation worst-case,
                                                          % this should be the open-loop pointing precision before
                                                          % acquisition
...
uplink_beacon_camera_telescope = components.Telescope( ...
    0.08,                                           ... % telescope diameter (m)
    'Wavelength',         uplink_beacon_wavelength, ...
    'Optical_Efficiency', 1,                        ... % telescope optical efficiency (1 is the default)
    'Focal_Length',       0.86,                     ... % telescope focal length (m)
    'Pointing_Jitter',    1e-3);                        % pointing precision (this is likely tied to the
                                                        % satellite's body pointing precision)


% there are many properties to every camera, which determine SNR and efficiency. to avoid specifying these, we will load a camera implementation which is pre-packaged.
...
% 1.3 orbit details
% there are many parameters which describe an orbit. we choose to use keplerian parameters. as well as these, we must also provide a time window to simulate (ideally, the satellite would pass the ground station in this time)
...
% provide a file which describes filter spectral performance. an example is given here. nativePathFrom provides path conversion for different OSs filter_file = utilities.nativePathFrom('Example Data/spectral filters/FBH780-10.xlsx'); spectral_filter = components.SpectralFilter('input_file', filter_file); alternatively, a dummy spectral filter can be created which has a brick-wall spectral response using

spectral_filter = components.IdealBPFilter(source.Wavelength, 5);

% 2.1.3 detector
% we provide details of some preset detectors. custom detectors can also be instantiated by providing their details. As well as performance data, the detector requires channel information to compute QBER.
...
% 2.2 beacon details
% we need to provide the other end of both beacon links. an uplink beacon laser and a downlink beacon camera
...
% we can specify beam divergence manually (in rads) rather than approximate it
...
%constructing a ground station requires details of the background light around it. this can be provided using a sky brightness store
