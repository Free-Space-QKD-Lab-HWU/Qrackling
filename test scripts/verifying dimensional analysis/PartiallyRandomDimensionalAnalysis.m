...
%ground station
SimGroundStation=Ground_Station('none',Detector,R_Tele,'N Pole',[89,0,0]);
...
%% plot
X=ND_Power.*(ND_Diameter.^2).*min(ND_Spectral_Width,ones(size(ND_Spectral_Width)));
Y=Reflected_Loss-Direct_Loss;
scatter3(X,Y,Jamming_Performance,'CDataMode','manual','CData',Jamming_Performance,'Marker','X');
ax=gca;
ax.XAxis.Scale='log';
xlabel('$\frac{P \lambda T}{h c} \times (\frac{D}{\lambda})^2 \times \min(\frac{\Lambda_{bob}}{\Lambda_{jan}},1)$','Interpreter','latex')
