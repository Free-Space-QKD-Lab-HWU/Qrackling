...
startTime = utilities.tleStartTime(tlestr);
...
startTime = utilities.tleStartTime(tle);
stopTime = startTime + hours(5);
sampleTime = 360;

[name, kepler_elements] = utilities.TLE2Kepler(TLE=tle);
[sma, ecc, inc, raan, aop, ta] = utilities.splat(kepler_elements);
...
startTime = utilities.tleStartTime(j{1});
stopTime = startTime + hours(5);
sampleTime = 360;

[names, kepler_elements] = utilities.TLE2Kepler(TLE=tle_4lines);
...
tledata
[name, ke] = utilities.TLE2Kepler(TLE=tledata)
