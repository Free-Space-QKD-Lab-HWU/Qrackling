...
transmission = libradtran.outputFromInputFile('~/Documents/MATLAB/lrt_test_data/transmission_in.txt')
radiances = libradtran.outputFromInputFile('~/Documents/MATLAB/lrt_test_data/radiances_in.txt')

dimensions = @(sized) [sized(2:end)];
to_mat = @(arr) reshape(arr(:, :), dimensions(size(arr)));

figure
plots.polarpcolor( ...
    transmission.azimuth, ...
    transmission.elevation, ...
    fliplr(flipud(to_mat(transmission.uu(1, :, :)))))

figure
plots.polarpcolor( ...
    radiances.azimuth, ...
    transmission.elevation, ...
    flipud(fliplr(to_mat(radiances.uu(1, :, :)))))

result.heading(result.within_elevation_limit)
result.elevation(result.within_elevation_limit)

%% env

environ = Environment( ...
    radiances.azimuth, ...
    fliplr(radiances.elevation - 90), ...
    radiances.lambda, ...
    radiances.uu, ...
    transmission.uu);

atten = environ.Interp( ...
    "attenuation", ...
    result.heading(result.within_elevation_limit), ...
    result.elevation(result.within_elevation_limit), ...
    500);
...
figure
polarplot( ...
    deg2rad(result.heading(result.within_elevation_limit)), ...
    result.elevation(result.within_elevation_limit))
set(gca(), ...
...
%% atmosphere spectral filter data...
